...
def download_missing_objects(new_uids_dict, lvis_annotations, file_removed_uids, processes, path_worksheet): 
  ...
  for objects_cat, uids_ in new_uids_dict.items():
    ...
    if removed_uids[objects_cat] and len(lvis_annotations[objects_cat])>10+ nb_to_download:
      print('yes')
      ...
      # get the next nb_to_download 
      for obj in range(1, nb_to_download+1): 
        annot = lvis_annotations[objects_cat][10+obj]
        ids.append(annot)
      ...
      # modify worksheet 
      save_new_worksheet('result_files/new_objects_folder2.xls', objects_cat, objects, nb_removed=len(removed_uids[objects_cat]))
